...
customer where discount_applied='Yes' and
...
select Round(AVG(purchase_amount),2)as "Avg Amount spent", shipping_type 
from customer where shipping_type IN('Standard','Express')
group by shipping_type ;

--Q5. Do subscribed customers spend more? Compare average spend and total revenue 
--between subscribers and non-subscribers.
select subscription_status,count(customer_id)as total_customers, Round(AVG(purchase_amount),2) as Avg_spent, ROUND(SUM(purchase_amount),2)
as total_revenue from customer 
group by subscription_status order by
total_revenue desc;
...
select age_group, count(purchase_amount)
as total_revenue
